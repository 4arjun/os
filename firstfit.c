...
void allocateFirstFit(struct Block* head, int processSize) {
    struct Block* temp = head;
    while (temp != NULL) {
        if (!temp->allocated && temp->size >= processSize) {
            temp->allocated = 1;
            temp->size = temp->size-processSize;
            printf("Process of size %d allocated to block of size %d\n", processSize, temp->size);
            return;
        }
        temp = temp->next;
    }
...
int main() {
    ...
    for (int i = 0; i < p; i++) {
        allocateFirstFit(memory, processes[i]);
    }
...
// enter process 3:112
// enter process 4:426
// Process of size 212 allocated to block of size 288
// Process of size 417 allocated to block of size 183
// Process of size 112 allocated to block of size 88
// Process of size 426 allocated to block of size 74
// Free Block
// 100->288->88->300->183->74->